...
python3 ../../../PARSE.py ../../config.yaml xyz pore.xyz input.dat -L 0.25 -r 0.1 --d_max 5 --d_step 0.2 --Voxel_dist 'Uniform'
...
#     - All files, inclduing PARSE.py, can be in the current folder, e.g., 'polymer_matrix.xyz', or a different folder, e.g., '{Path-to-file}/polymer_matrix.xyz'
...
#   --print_eff:            defines how much information is printing while PARSE.py is running. Typically 1, but 2 is useful for trouble shooting memory errors or significant slowdowns in compute time.
#   --print_xyz:            defines whether PARSE.py generates xyz files to visualize the probe-occupiable volume that is analyzed. Typically False to conserve harddrive space (these xyz can get large for small L_voxel or large simulation boxes).
