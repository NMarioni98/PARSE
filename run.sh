...
python3 ../PrO-VAT.py ../config.yaml xyz polymer_matrix.xyz input.dat --Voxel_dist Uniform
...
# Load in the trajectory data creating a PrO-VAT.hdf5 data file
python3 ../PrO-VAT.py ../config.yaml gmx md.gro md.tpr -m 'moltype Other' -s 'percolated' --Voxel_dist Uniform
...
# PrO-VAT.py takes the following inputs
#    python3 {PATH}/PrO-VAT.py {Path}/{YAML config file} gmx {PATH}/{input .XTC/.TRR/.GRO file} {PATH}/{input .TPR file}
#     - "-m 'moltype MOL'", "-s 'percolated'", and "--Voxel_dist Uniform" overwrite the "system_name", "solvent_name" , and "Voxel_dist" variables in the yaml config file, respectively
